...
const UI_TEXT = {
  en: {
    unnamedLocation: 'Unnamed location', settlement: 'Settlement', modern: 'Modern:', administrativeNote: 'Administrative Note', close: 'Close', showAllTowns: 'Show all towns', failedToLoadMap: 'Failed to load map',
  ...
  'zh-hant': {
    unnamedLocation: '未命名地點', settlement: '地點', modern: '現代位置：', administrativeNote: '政區考釋', close: '關閉', showAllTowns: '顯示全部地點', failedToLoadMap: '地圖加載失敗',
  ...
  'zh-hans': {
    unnamedLocation: '未命名地点', settlement: '地点', modern: '现代位置：', administrativeNote: '政区考释', close: '关闭', showAllTowns: '显示全部地点', failedToLoadMap: '地图加载失败',
...
const PROVINCE_LAYERS = ['province-fill', 'province-line'];
...
const ARCHIVE_SOURCES = ['towns', 'provinces', 'yellow-river-old-course'];
...
async function fetchProvinces() {
  const raw = await fetch(url('/mapbase/All_Provinces.json')).then(r => r.json());
...
function convertGeometryToLngLat(geometry, shouldConvert) {
  if (!shouldConvert || !geometry?.coordinates) return geometry;
...
// LAYER BUILDING 
function addLayers(provincesGeoJSON, yellowRiverOldCourseGeoJSON = yellowRiverOldCourseCache) {
...
function restoreArchiveLayersAfterStyleChange() {
  ...
  addLayers(provincesGeoJSONCache, yellowRiverOldCourseCache);
  removeKingdomLegend();
...
export async function initInteractiveMap(options = {}) {
  ...
  try {
    ...
    const [provincesGeoJSON, townsData, yellowRiverOldCourseGeoJSON] = await Promise.all([
      fetchProvinces(),
      fetchTowns(),
    ...
    map.on('load', () => {
      addLayers(provincesGeoJSON, yellowRiverOldCourseGeoJSON);
